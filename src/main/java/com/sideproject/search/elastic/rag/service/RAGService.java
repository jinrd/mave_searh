...
public class RAGService {
    ...
    public String getanswer(String question) throws IOException {
        ...
        SearchRequest searchRequest = new SearchRequest.Builder()
            .index(esIndexName)
            .knn(k -> k.field("contentVector").queryVector(queryVectorList).k(2L).numCandidates(10L))
            .build();

        SearchResponse<CarManual> searchResponse = elasticsearchClient.search(
            searchRequest,
            CarManual.class
        );
        ...
        ChatRequestVO chatRequest = new ChatRequestVO(llmModelVersion, messages);

        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        headers.setBearerAuth(llmApiKey);

        HttpEntity<ChatRequestVO> httpEntity = new HttpEntity<>(chatRequest, headers);

        try {
            ChatResponseVO chatResponse = restTemplate.postForObject(
                llmApiUrl,
                httpEntity,
